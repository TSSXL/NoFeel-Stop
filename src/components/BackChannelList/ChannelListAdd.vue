    ...
    <el-main style="width:70%;">
      <el-form :model="addForm" :rules="rules" ref="addForm" label-width="150px" class="demo-addForm">
        <el-form-item label="规则名称：" prop="ActivityName">
          <el-input v-model="addForm.ActivityName" style="width:200px;" placeholder="请输入新增不同周期礼"></el-input>
        </el-form-item>
        <el-form-item label="是否启用：" prop="IsClose">
          <el-radio v-model="addForm.IsClose" label="0">是</el-radio>
          <el-radio v-model="addForm.IsClose" label="1">否</el-radio>
        </el-form-item>
        <el-form-item label="活动时间：" prop="ActivityTime">
          <el-date-picker v-model="addForm.ActivityTime" value-format="yyyy-MM-dd" type="daterange">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="优惠券：" prop="addValue">
          ...
          <div class="addList">
            启用
            <input class="checkout" type="checkbox" v-model="addForm.addValue.Checked">
            <!-- <el-checkbox v-model="addForm.addValue.Checked"></el-checkbox> -->
            <el-autocomplete v-model="addForm.addValue.CouponNo" :fetch-suggestions="querySearch" placeholder="请检索选择" @select="handleSelect">
        ...
        <el-form-item label="">
          <div class="addList" v-for="(item, index) in addForm.domains" :key="index">
            启用
            <input class="checkout" type="checkbox" v-model="item.Checked">
            <el-input v-model="item.CouponNo" style="width:200px;" disabled></el-input>
...
export default {
  ...
  methods: {
    //新增 删除  券
    addDomain() {
      console.log(this.addForm.addValue);
      if (this.addForm.addValue.CouponNo == undefined) {
        this.$message({
          showClose: true,
          type: "warning",
          message: "请选择正确的优惠券"
        ...
        if (
          JSON.stringify(this.addForm.domains).indexOf(
            JSON.stringify(this.addForm.addValue.CouponNo)
          ) > 0
        ) {
          this.$message({
            showClose: true,
          ...
          this.quanNum++;
          this.addForm.domains.unshift(this.addForm.addValue);
          this.addForm.addValue = {};
        }
      }
    },
    removeDomain(item) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            var infoObj = Object.assign({}, this.addForm);
            var para = {};
            // 将token传入参数中
            ...
            if (this.addForm.domains.length < 1) {
              ...
              var arr = [];
              for (let j = 0; j < this.addForm.domains.length; j++) {
                const itemele = this.addForm.domains[j];
    ...
    handleSelect(item) {
      // this.addForm = item
      this.addForm.addValue.ActivityName = item.ActivityName;
      this.addForm.addValue.CouponNo = item.CouponNo;
      this.addForm.addValue.ID = item.ID;
      this.addForm.addValue.Checked = true;
    },
    // 立即发放
    handleSend(id) {
  ...
  beforeMount() {
    this.quanNum = this.addForm.domains.length;
    this.loadAll();
  },
  mounted() {
